...
class Pardot_Plugin {
	...
	function admin_init() {
		...
		if ( 'plugins.php' == $pagenow ) {
			add_filter( 'plugin_action_links_pardot/pardot.php', array( $this, 'plugin_action_links' ) );
	 ...
	 * @since 1.0.0
	 */
	static function get_form_body( $args = array() ) {
		$body_html = false;
		/**
	 ...
	 * @since 1.0.0
	 */
	static function get_api_key( $auth = array() ) {
	 ...
	 * @since 1.0.0
	 */
	static function get_campaigns( $args = array() ) {
	 ...
	 * @since 1.0.0
	 */
	static function get_account( $args = array() ) {
	 ...
	 *
	 * Attempt to get the value from the cache before actually calling the API.
	 * Sets $args['new_api_key'] as a callback in case the API key times out and has to be renewed.
	 *
	 * @static
	...
	/**
	 * "Private" method to be used as a callback to set the API Key when the api key has expired.
	 * This method is passed as a callback in $args['new_api_key'] to enable non-coupling of the
	 * Pardot_API class with WordPress and/or this plugin.
	 *
	 * @static
	 * @param string $api_key Value returns as a valid API key from Pardot login API.
	 *
	 * @since 1.0.0
	 */
	static function _new_api_key( $api_key ) {
		Pardot_Settings::set_setting( 'api_key', $api_key );
	}
	 ...
	 * @since 1.0.0
	 */
	static function set_cache( $key, $value, $set_transient = true ) {
		wp_cache_set( $key, $value, 'pardot' );
		if ( $set_transient )
			set_transient( "pardot_{$key}", $value, self::$cache_timeout );
	}
